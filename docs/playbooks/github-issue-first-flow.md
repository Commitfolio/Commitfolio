...
- 작업 브리핑에는 Commitfolio `madmax` 실행 규칙을 포함한다. 새 OMX/Codex 런타임 세션은 기본적으로 `omx --madmax` 또는 해당 명령의 `--madmax` 조합으로 시작한다.
...
- 새 OMX 런타임 세션을 열어 작업한다면 `omx --madmax`를 사용한다. 이미 full-access/equivalent 권한 세션이면 재실행하지 않고 madmax 규칙이 충족되었다고 보고한다. `omx --madmode`는 사용하지 않는다.
